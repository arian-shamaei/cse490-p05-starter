...
* From here, use Claude Code as the harness calling the MCP. Ask Claude Code to fill in its permissions with the following prompt: "In .claude/settings.json, allow the four Blender tools the ride needs: reading the scene, reading one object, taking a screenshot, and running a script. Deny every other tool named in RULES.md." Claude Code also runs your gate before every call, so the script check from step 2 is already in place. One of the four tools runs any Python it is handed; which one?
* Ask Claude Code to build the ride with the following prompt: "Read fixtures/scene_brief.md and build the scene it describes in Blender, with my setting: [your setting]. Then animate the ride, two to five seconds, plus this extra motion of mine: [your beat]." Does the pelican ride?
...
* Ask Claude Code to render with the following prompt: "Render the animation with the Workbench engine, one PNG per frame, into ride/frames." A refused step ends an ask, so the render is its own ask. Did the legitimate work go through?
