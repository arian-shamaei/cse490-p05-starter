...
* From here, use Claude Code as the harness calling the MCP. It already knows where the Blender server is (.mcp.json) and already runs your gate from step 2 before every Blender call (.claude/settings.json); what it lacks is a list of which tools it may use. Ask Claude Code to fill in its permissions with the following prompt: "In .claude/settings.json, allow get_scene_info, get_object_info, get_viewport_screenshot and execute_blender_code, and deny every other tool named in RULES.md." One of those four runs any Python it is handed; which one? 
