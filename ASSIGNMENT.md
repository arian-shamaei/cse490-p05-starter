...
* Once you are satisfied with your output, export the animation. Ask Claude Code to render with the following prompt: "Render the animation with the Workbench engine, one PNG per frame, into ride/frames." 
* Run python checker.py. It checks ride/ride.mp4 (and makes it from your frames if it is missing), tells you which promises hold and which don't yet, and writes your bundle to submission/. Fix what it names.
